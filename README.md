...
The Appoooh MRA provides three action buttons (Button 1–3) and defaults to DIP bytes `60,ff`. It exposes Coin A, Coin B, Demo Sounds, Cabinet and Difficulty.

## Robo Wres 2001

The core also accepts the parent Robo Wres set through [Robo Wres 2001.mra](releases/Robo%20Wres%202001.mra) at MiSTer ROM index 1. It uses the Sega 315-5179 opcode/data decryption, a separate 24 KiB CPU ROM map, 32 KiB graphics planes, and the 32 KiB ADPCM ROM. Its three action buttons are named Punch, Grapple and Special, in that order. The MRA exposes Coin A, Coin B, Demo Sounds and Language; SW1:7 is left unused as in MAME. The default switch bytes are `e0,ff`. The packed CPU ROM bytes at `0x12000–0x15fff` populate the two switched 8 KiB pages at `a000–dfff`.
